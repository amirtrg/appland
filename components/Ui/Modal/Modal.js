...
const Modal = ({ children, className, onExit,isShown }) => {
  ...
  return (
    <>
      ...
      className={`z-50 fixed top-1/2 left-0 right-0 transform -translate-y-1/2  mx-8  max-w-lg transition-all ease-linear delay-500 duration-300 bg-white dark:bg-[#111]/30 dark:border dark:border-white/40 dark:backdrop-blur dark:text-white ${className}`}>
       {children}
      </Card>
      <motion.div 
      initial={{opacity:0}}
